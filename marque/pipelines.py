...
class MarquePipeline:

    # Database setup
    conn = sqlite3.connect('marque.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `marque`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM marque WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO marque (date, title, link, content)"
